...
The "binary space partiion" tree is the core data structure used in Doom's [software rendering engine](https://doomwiki.org/wiki/Doom_rendering_engine).

| BSP tree type | ZIP compressed version | Magic number   | ZIP magic number |
|---------------|------------------------|----------------|------------------|
| Vanilla       |                        |                |                  |
...
| `uint16_t` | First segment |

#### Vanilla segment (`doom_seg_t`), 12 bytes
Pulled from https://doomwiki.org/wiki/Seg
...
Pulled from https://www.sbsoftware.com/files/DeePBSPV4specs.txt
...
Pulled from https://zdoom.org/w/index.php?title=Node#ZDoom_extended_nodes
...
| `uint32_t` | Index of starting vertex |
| `uint32_t` | Index of ending vertex |
| `uint32_t` | Index of associated linedef |
| `uint8_t`  | False (0) if on the same side as linedef, true (1) if oppposite side |
