...
Changes in V2.27  (TEST ONLY)
-----------------------------

 - experimental new method for fast mode (-f), which does not
   require existing nodes.  Instead it works by simply picking
   the horizontal or vertical partition which is closest to a
   perfect binary division.  On large maps it can be upto six
   times faster.
